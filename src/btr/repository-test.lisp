...
(deftest repository-test parse-units ()
...
(deftest repository-test making-group ()
  (let ((group (make-group "group")))
    (!equal (entity-name group) "group")
    (!eq (entities group) ())))

(deftest repository-test group-with-files ()
  (let ((group (make-group "group"))
	(unit (make-unit "unit")))
    (add-entity unit group)
    (mapc #'!entity= (entities group) (list unit))))

(deftest repository-test group-with-multiple-entities ()
  (let ((group (make-group "group"))
...
(deftest repository-test parsing-groups ()
  (let ((repo (read-repository (echo "<repository version=\"0.1\">"
				     "  <group name=\"g1\">"
				     "    <unit name=\"u1\"/>"
				     "  </group>"

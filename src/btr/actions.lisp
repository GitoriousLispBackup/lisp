...
(defun add-unit (path repository)
  (let ((name (path-to-string (copy-path path :new-path '(:relative)))))
    (add-entity (make-unit name :files (list name)) (find-group (rest (path-path path)) repository))))
...
(defun group-names (group path &optional recursive-p)
  (labels ((entity-path (entity)
	     (as-relative-path (path+ path (path-from-string (entity-name entity))) (current-directory)))

...
(deftest state-machine-test state-machine-creating ()
  (let ((machine (create-state-machine sample-lexic)))
    (!oequal (slot-value machine 'values)
	     (list nil nil nil (get-lexeme 'sample-lexeme)))
...
(deftest state-machine-test machine-with-minimal ()
  (let ((machine (create-state-machine min-lexic)))
    (!eq (lexeme-name (machine-value machine "\"bla-bla\"")) 'c-string)
    (!eq (lexeme-name (machine-value machine "\"\"bla-bla\"")) nil)))

(deflexeme skipped-space (:star (:char #\Space)) :skipped t)
(deflexeme value (:positive (:letter)))

(deflexic skip-lexic skipped-space value)

(deftest state-machine-test skip-test ()
